...
function collapseItem(theme, ownerState) {
  ...
  const { success, white, text, transparent, dark, info} = palette;
  ...
  return {
    ...
    color: active ? info.main : dark.main,
    display: "flex",
    alignItems: "center",
    width: "100%",
...
function collapseIconBox(theme, ownerState) {
  ...
  const { white, info, light, gradients , dark,  success} = palette;
  const { md } = boxShadows;
  const { borderRadius } = borders;
  const { pxToRem } = functions;
  ...
  return {
    background: () => {
      if (active) {
        return color === "default" ? info.main : dark.main;
      }
    ...
    [breakpoints.up("xl")]: {
      background: () => {
        ...
        if (!active) {
          ...
          background = transparentSidenav ? dark.main : dark.main;
        }
