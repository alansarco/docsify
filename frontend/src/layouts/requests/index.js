...
import Add from "layouts/requests/Add";
...
function DocRequest() {
  ...
  const ReloadTable = () => {
    axios.get(apiRoutes.requestRetrieve + '?page=' + page, { params: { filter }, headers })
    .then(response => {
  ...
  const fetchNextPrevTasks = (link) => {
    ...
    axios.get(apiRoutes.requestRetrieve + '?page=' + nextPage, { params: { filter }, headers })
    .then(response => {
      setFetchdata(response.data.requests);
      setReload(false);      
    })
        ...
        <SoftBox p={2}>
          <SoftBox >   
            <SoftBox className="px-md-4 px-3 py-2" display="flex" justifyContent="space-between" alignItems="center">
              <SoftBox>
                <SoftTypography className="text-uppercase text-secondary" variant="h6" >Document Request List</SoftTypography>
              </SoftBox>
              <SoftBox display="flex">
                ...
                <SoftBox display="flex" >
                  <SoftButton onClick={() => setRendering(3)} className="ms-2 py-0 px-3 d-flex rounded-pill" variant="gradient" color="info" size="small" >
                    <Icon>add</Icon> Request Document
                  </SoftButton>
                </SoftBox>
                }
              </SoftBox>
