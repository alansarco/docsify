...
import Add from "layouts/documents/Add";
...
function Documents() {
  ...
  const ReloadTable = () => {
    axios.get(apiRoutes.docRetrieve + '?page=' + page, { params: { filter }, headers })
    .then(response => {
  ...
  const fetchNextPrevTasks = (link) => {
    ...
    axios.get(apiRoutes.docRetrieve + '?page=' + nextPage, { params: { filter }, headers })
    .then(response => {
      setFetchdata(response.data.organizations);
      setReload(false);      
    })
        ...
        <SoftBox p={2}>
          <SoftBox >   
            <SoftBox className="px-md-4 px-3 py-2" display="flex" justifyContent="space-between" alignItems="center">
              <SoftBox>
                <SoftTypography className="text-uppercase text-secondary" variant="h6" >Document List</SoftTypography>
              </SoftBox>
