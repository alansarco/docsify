...
import DataContainer from "layouts/documents/components/DataContainer";
import Add from "layouts/documents/components/Add";

import Table from "layouts/documents/data/table";
import { tablehead } from "layouts/documents/data/head";  
...
function Documents() {
    ...
    useEffect(() => {
      if (searchTriggered) {
        setReload(true);
        axios.post(apiRoutes.documentRetreive + '?page=' + 1, formData, {headers})
          .then(response => {
    ...
    const ReloadTable = () => {
        axios.post(apiRoutes.documentRetreive + '?page=' + page, formData, {headers})
        .then(response => {
        setFetchdata(response.data.documents);
        passToSuccessLogs(response.data, currentFileName);
        setReload(false);      
    ...
    const handleSubmit = async (e) => {
        ...
        setReload(true);      
        try {
            const response = await axios.post(apiRoutes.documentRetreive + '?page=' + 1, formData, {headers});
  ...
  const fetchNextPrevTasks = (link) => {
    ...
    axios.post(apiRoutes.documentRetreive + '?page=' + nextPage, formData, {headers})
    .then(response => {
      setFetchdata(response.data.documents);
      passToSuccessLogs(response.data, currentFileName);
      setReload(false);      
          ...
          <SoftBox p={2}>
            <SoftBox >   
              <SoftBox className="px-md-4 px-3 py-2 d-block d-sm-flex" justifyContent="space-between" alignItems="center">
                <SoftBox>
                  <SoftTypography className="text-uppercase text-dark" variant="h6" >Document List</SoftTypography>
                </SoftBox>
