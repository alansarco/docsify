...
import Table from "layouts/licenses/data/table";
import { tablehead } from "layouts/licenses/data/head";
import Add from "layouts/licenses/Add";
...
function Licenses() {
  ...
  const [showFilter, setShowFilter] = useState(false);
    ...
    useEffect(() => {
      if (searchTriggered) {
        setReload(true);
        axios.post(apiRoutes.licenseRetrieve + '?page=' + 1, formData, {headers})
          .then(response => {
            setFetchdata(response.data.licenses);
    ...
    const ReloadTable = () => {
      axios.post(apiRoutes.licenseRetrieve + '?page=' + page, formData, {headers})
      .then(response => {
      setFetchdata(response.data.licenses);
      passToSuccessLogs(response.data, currentFileName);
      setReload(false);      
    ...
    const handleSubmit = async (e) => {
        ...
        setReload(true);      
        try {
            const response = await axios.post(apiRoutes.licenseRetrieve + '?page=' + 1, formData, {headers});
  ...
  const fetchNextPrevTasks = (link) => {
    ...
    axios.post(apiRoutes.licenseRetrieve + '?page=' + nextPage, formData, {headers})
    .then(response => {
      setFetchdata(response.data.licenses);
      passToSuccessLogs(response.data, currentFileName);
      setReload(false);      
                  ...
                  <Grid container spacing={1} py={1} pb={2}>  
                      <Grid item xs={12}>
                          ...
                          <SoftBox className="my-auto">
                            ...
                            <select className="form-select form-select-sm text-secondary cursor-pointer rounded-5 border" name="filter_price" value={formData.filter_price} onChange={handleChange} >
                              <option value="">-- Select --</option>
                              {minPaymenSelect && minPaymenSelect.map((max) => (
