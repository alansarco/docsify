...
import { tablehead } from "layouts/task-active-student/data/head";  
...
function StudentActiveRequest() {
    ...
    useEffect(() => {
      if (searchTriggered) {
        setReload(true);
        axios.post(apiRoutes.activeStudentRequestRetreive + '?page=' + 1, formData, {headers})
          .then(response => {
    ...
    const ReloadTable = () => {
        axios.post(apiRoutes.activeStudentRequestRetreive + '?page=' + page, formData, {headers})
        .then(response => {
        setFetchdata(response.data.requests);
        passToSuccessLogs(response.data, currentFileName);
        setReload(false);      
    ...
    const handleSubmit = async (e) => {
        ...
        setReload(true);      
        try {
            const response = await axios.post(apiRoutes.activeStudentRequestRetreive + '?page=' + 1, formData, {headers});
  ...
  const fetchNextPrevTasks = (link) => {
    ...
    axios.post(apiRoutes.activeStudentRequestRetreive + '?page=' + nextPage, formData, {headers})
    .then(response => {
      setFetchdata(response.data.requests);
      passToSuccessLogs(response.data, currentFileName);
      setReload(false);      
...
export default StudentActiveRequest;